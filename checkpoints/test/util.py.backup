...
def farthest_point_sample(xyz, npoint, RAN=True):
    '''
    Input:
        xyz: input point cloud, [B, N, C]
        npoint: number of samples
    Output:
        idx: sampled points indices, [B, npoint]
    '''
    device = xyz.device
    B, N, C = xyz.shape
    ...
    for i in range(npoint):
        ...
        idx_temp = xyz[batch_indices, farthest, :].view(B, 1, 3)
        dist = torch.sum((xyz-idx_temp)**2, -1)
        mask = dist < distance
...
def sample_and_group(npoint, radius, nsample, xyz, points):
    ...
    fps_idx = farthest_point_sample(xyz=xyz, npoint=npoint) # [B, npoint]
